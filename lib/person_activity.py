...
class PersonActivity(object):
	...
	def __init__(self, root, download=True,
		...
		if os.path.exists(os.path.join(self.root, self.data_file)):
			path = os.path.join(self.root, self.data_file)
		else:
			if download:
				self.download()
		...
		self.data = torch.load(path, map_location=self.device, weights_only=True)
