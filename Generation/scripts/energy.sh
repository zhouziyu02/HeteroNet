...
if [[ ! -f "$DATA_ROOT/energy_data.csv" ]]; then
  printf 'Missing real Energy dataset: %s/energy_data.csv\n' "$DATA_ROOT" >&2
...
for seq_len in 12 24 36; do
  for missing in 0.3 0.5 0.7; do
    out_dir="outputs/energy/len${seq_len}_miss${missing}_seed${SEED}"
    python3 train_irregular_generation.py \
      --gpu "$GPU" --seed "$SEED" --dataset energy \
      --generator heteronet_conditioned_diffusion --data_root "$DATA_ROOT" \
